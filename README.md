...
The deterministic Graham scoring foundation lives in `src/lib/scoring`. It currently defines pure TypeScript contracts, default product-plan thresholds, stock labels, portfolio-fit labels for later portfolio scoring work, rule-check results, score-layer results, and explicit missing/insufficient cached-data states. The module does not call Supabase, Next.js rendering, market-data providers, or AI services; later Milestone 6 work can map cached price and fundamentals rows into these contracts before calculating scores.
