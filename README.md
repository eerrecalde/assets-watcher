...
The deterministic Graham scoring foundation lives in `src/lib/scoring`. It defines pure TypeScript contracts, default product-plan thresholds, stock labels, portfolio-fit labels for later portfolio scoring work, rule-check results, score-layer results, explicit missing/insufficient cached-data states, pure Graham Number plus margin-of-safety calculation helpers, and a valuation scoring layer for cached P/E, P/B, Graham Number, and latest cached price data. The valuation layer calculates margin of safety from cached price and Graham Number when both are valid, excludes unavailable inputs from pass/fail scoring, and returns deterministic `Attractive`, `Reasonable`, `Watch`, `Expensive`, or `Insufficient Data` valuation labels with rule-level explanations. The module does not call Supabase, Next.js rendering, market-data providers, or AI services.
